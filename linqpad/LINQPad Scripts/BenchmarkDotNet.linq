...
// This script can be #load-ed into other scripts for BenchmarkDotNet support. *V=1.7*
...
#LINQPad optimize+   // Enable compiler/JIT optimizations when benchmarking.
                     // You can override this in your script with #LINQPad optimize-
...
/// <summary>This method runs when you choose "Benchmark Selected Code" from the Script menu (Ctrl+Shift+B or Command-Shift-B).
...
Summary[] RunBenchmark (IConfig config, bool allowDumping = false)
{
	...
	using (allowDumping ? null : ExecutionEngine.SuspendDump())
	{
		...
		// We need to ensure that the power plan is restored should the script be prematurely cancelled.
		var powerRestorer = new PowerPlanRestorer();
		this.ScriptCancelToken.Register (() => 
		{			
...
namespace LINQPad.Benchmark
{
	sealed class LiveSummaryLogger : ILogger, IDisposable
	{
		...
		public LiveSummaryLogger (IConfig config)
		{
			...
			string.Join (Environment.NewLine, envInfo.Skip(1)).Dump (envInfo.FirstOrDefault());

			new Hyperlinq (() => Util.OpenScript (Path.Combine (Util.MyScriptsFolder, "BenchmarkDotNet.linq")), "Customize benchmarking behavior", true).Dump();			
